...
SECTION_PATTERNS = {
    "links": re.compile(r"^(links?|url|website|web\s*site|resources?|https?://|www\.)\b", re.I),

    "visuals": re.compile(
        r"^(visuals?|image|picture|photo|diagram|chart|table|video|watch|look\s+at|comparison\s+chart)\b",
        re.I,
    ),
    ...
    "speaking": re.compile(
        ...
        r"role\s*play|debate|presentation|students\s*discuss|students\s*must\s*say|"
        r"real[-\s]*life\s*problem\s*solving|comparison\s+chart\s*\(speaking\))\b",
        re.I,
    ),
    ...
    "games": re.compile(r"^(game|games|vocab\s*match|leaderboard)\b", re.I),

    "homework": re.compile(r"^(homework|home\s*task|assignment|at\s*home|self[-\s]*study)\b", re.I),
...
def _smart_task_classifier(text: str) -> str | None:
    ...
    if "comparison chart" in low and "speaking" in low:
        return "speaking"

    if "students must say" in low or "superlatives" in low:
        return "speaking"
...
def _classify_section_title(text: str) -> str | None:
    ...
    for cat in ORDERED_CATEGORIES:
        for kw in CATEGORY_KEYWORDS.get(cat, []):
            ...
            if kw_low and (low == kw_low or low.startswith(kw_low + ":")):
                return cat
...
def _guess_category_from_content(text: str) -> str | None:
    ...
    if "comparison chart" in low or "students must say" in low or "superlatives" in low:
        return "speaking"

    if re.search(r"\bdiscuss\b|\bspeak\b|\bpresentation\b|\bpair work\b|\bgroup work\b|\bsentences using\b", low):
        return "speaking"

    if re.search(r"\blisten\b|\baudio\b|\btrack\b|\brecording\b", low):
        return "listening"

    if re.search(r"\bread\b|\bpassage\b|\barticle\b|\breading text\b", low):
        return "reading"

    if re.search(r"\b(customer|delivery|product|payment|definition|definitions|means|match the word|word bank|vocabulary)\b", low):
        return "vocabulary"
...
def _split_items_to_categories(
    ...
    def flush():
        ...
        target_cat = current_cat or _guess_category_from_content(text) or "speaking"
    ...
    if not result:
        ...
        if full_text:
            guessed = _guess_category_from_content(full_text) or "speaking"

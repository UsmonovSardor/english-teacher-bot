...
Admin sessions use in-memory storage (fast, no wipe on redeploy issues).
"""
...
# In-memory admin sessions — persists within process lifetime
...
# ── Admin (IN-MEMORY — not affected by DB wipes) ─────────────────────────────
...
def lesson_leaderboard(lesson_id: int, limit=10):
    with _db() as c:
        return c.execute("""
            SELECT s.full_name, s.username, q.score, q.total,
            ...
            ORDER BY pct DESC, q.at ASC LIMIT ?""", (lesson_id, limit)).fetchall()

def global_leaderboard(limit=10):
    with _db() as c:
        return c.execute("""
            SELECT s.full_name, s.username, COUNT(q.id) quiz_count,
                   AVG(CAST(q.score AS FLOAT)/q.total*100) avg_pct, SUM(q.score) total_score
            FROM quiz_scores q JOIN students s ON q.chat_id=s.chat_id
            WHERE q.total>0
            GROUP BY q.chat_id ORDER BY avg_pct DESC LIMIT ?""", (limit,)).fetchall()

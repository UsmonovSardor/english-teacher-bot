...
CAT_LABEL = {key: lbl for lbl, key in db.CATEGORIES if hasattr(db, "CATEGORIES")}
...
async def show_lesson(update: Update, context: ContextTypes.DEFAULT_TYPE, lid: int):
    ...
    if not lesson:
        await update.callback_query.edit_message_text("⚠️ Lesson not found."); return
...
async def new_lesson_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not db.is_admin(update.effective_chat.id):
        await update.callback_query.answer("Access denied."); return ConversationHandler.END
    await update.callback_query.answer()
    await update.callback_query.edit_message_text(
...
async def new_lesson_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not title:
        await update.message.reply_text("⚠️ Title required."); return State.ADD_LESSON
...
async def receive_doc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not lid or not db.is_admin(update.effective_chat.id): return
    ...
    try:
        ...
        if not parsed:
            await msg.edit_text("⚠️ No recognizable content found."); return

        total = 0
        for cat, blocks in parsed.items():
            for i, b in enumerate(blocks):
                db.add_content(lid, cat, b, i); total += 1
...
async def rename_lesson(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not lid: return ConversationHandler.END

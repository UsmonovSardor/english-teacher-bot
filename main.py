...
async def cmd_start(update, context):
    ...
    except Exception as e: logger.error("DB upsert: %s", e)
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("👨‍💼 Admin Panel", callback_data="admin"),
        InlineKeyboardButton("📚 Student Zone", callback_data="student"),
    ]])
    await update.message.reply_text(f"👋 *Hello, {u.first_name}!*\n\nWelcome to *Lingua Bot* 🎓\nChoose your role:",
        parse_mode="Markdown", reply_markup=kb)

async def _deny(update, msg="⛔ Access denied."):
...
async def callback_route(update, context):
    ...
    try:
        if data == "student":
            ...
            await update.callback_query.edit_message_text("✏️ *Rename Lesson*\n\nSend the new title:", parse_mode="Markdown")
        ...
        try: await update.callback_query.answer("⚠️ Error. Try again.", show_alert=True)
        except: pass
...
async def post_init(app):
    ...
    db.init_db(); logger.info("DB ready")
    try:
        from telegram import BotCommand, MenuButtonCommands
        await app.bot.set_my_commands([BotCommand("start","Welcome"),BotCommand("help","Help")])
